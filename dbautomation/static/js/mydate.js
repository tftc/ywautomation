...
L_calendar.prototype={
	...
	CreateHTML:function(){
		...
		for(var i=0;i<this.L_WDay.length;i++){
			htmlstr+="<li id=\"L_calendar-day_"+i+"\" style=\"background:white\" onmouseover=\"this.style.background='#ccff00'\"  onmouseout=\"parent."+this.NewName+".SetDisplayCor("+i+",this)\"></li>\r\n";
		}
		htmlstr+="</ul>\r\n";
		...
		stylestr+="#L_calendar{border:1px solid gray;width:158px;padding:1px;height:180px;z-index:9998;text-align:center}\r\n";
		...
		stylestr+="#L_calendar-control{height:25px;z-index:9998;}\r\n";
			stylestr+="#L_calendar-today{cursor:pointer;float:left;width:63px;height:20px;line-height:20px;margin:1px;text-align:center;background:#ccff00}"
		stylestr+="</style>";
	...
	SetDay:function (yy,mm)   //主要的写程序**********
	{
  		...
  		for (i = 0; i < 39; i++)
		{
			...
			{ 
				...
				da.style.background=(yy == new Date().getFullYear()&&month==new Date().getMonth()+1&&day==new Date().getDate())? "#ccff00":"white";
				if(this.InputDate!=null){
					if(yy==this.InputDate.getFullYear() && month== this.InputDate.getMonth() + 1 && day==this.InputDate.getDate()){
						da.style.background="#FFFF00";
						}
					}
      		}
  		}
