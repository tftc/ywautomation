  ...
  // ------ ONLY FOR DEMONSTRATION ------- //
	...
	if($('.flot').length > 0){
		...
		if($('.flot.flot-line').length > 0){
			$.plot($(".flot.flot-line"), [ {label: " guests", data: sin}, {label: " members", data: cos} ] , options);
		}

		if($(".flot-pie").length > 0){
			$.plot($(".flot-pie"), [ {label: " guests", data: 5}, {label: " members", data: 10},{label: "Label #3", data: 3},{label: "Label #4", data: 7} ] , options2);
		}
		var d1 = [];
